# 人类营养 / Human nutrition

<!-- TeachAny KP: kp_id=kp-biology-cam-igcse-human-nutrition node_id=biology-cam-igcse-human-nutrition subject=biology stage=igcse -->

## 元数据

| 字段 | 值 |
| --- | --- |
| kp_id | `kp-biology-cam-igcse-human-nutrition` |
| node_id | `biology-cam-igcse-human-nutrition` |
| 知识点 | 人类营养 |
| 英文名 | Human nutrition |
| 学科 | 生物 (biology) |
| 学段 | IGCSE (igcse) |
| 年级 | 10 |
| 课标系统 | 剑桥国际课程 |
| 领域 | 营养与运输 / Nutrition & Transport / Nutrition & Transport |

## 课标原文（Cambridge International）

### 课标原文

1 Describe diffusion as the net movement of particles from a region of their higher concentration to a region of their lower concentration (i.e. down a concentration gradient), as a result of their random movement

3 State that some substances move into and out of cells by diffusion through the cell membrane

4 Describe the importance of diffusion of gases and solutes in living organisms

5 Investigate the factors that influence diffusion, limited to: surface area, temperature, concentration gradient and distance

## 知识图谱关系

### 前驱知识

- `biology-cam-igcse-biological-molecules`

## 课标摘要

1 Describe diffusion as the net movement of particles from a region of their higher concentration to a region of their lower concentration (i.e. down a concentration gradient), as a result of their ra...；3 State that some substances move into and out of cells by diffusion through the cell membrane；4 Describe the importance of diffusion of gases and solutes in living organisms；5 Investigate the factors that influence diffusion, limited to: surface area, temperature, concentration gradient and distance
