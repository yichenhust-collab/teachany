...
| **⑩ 标准历史地图模块**（v7.7 新增） | 历史 / 地理课件提到"疆域 / 战役 / 路线 / 政区"必须使用 `scripts/teachany-historical-map.js` 渲染，**地图必须引用 `skill/assets/historical-{china,world}/*.geojson` 规范底图**，不再用纯示意 SVG | 引入 `scripts/teachany-historical-map.{css,js}`；写 `<div data-teachany-map="qin-dynasty" data-teachany-map-scope="china" data-teachany-map-title="秦统一中国（前221）"><script type="application/json" data-teachany-map-config>{"annotations":[{"type":"city","coord":[108.95,34.27],"label":"咸阳","role":"capital"}]}</script></div>`；模块自动加载对应朝代 GeoJSON、按 `LEVEL` 分层（country / prefecture）、绘制都城/战役/路线标注，并提供图层开关 | ⛔ **严禁**用纯手画 SVG / Canvas 块状图当作"历史地图"；⛔ 必须使用既有规范朝代 ID（见 `skill/assets/historical-china/` 与 `historical-world/` 文件名）；新朝代/区域需先补 GeoJSON 再使用 |
