...
| **⑩ 标准历史地图模块**（v7.7 新增 / v7.7.2 升级为 Leaflet / v7.7.3 默认加载彩色阴影地形底图）| 历史 / 地理课件提到"疆域 / 战役 / 路线 / 政区 / 朝代变迁"必须使用 `scripts/teachany-historical-map.js` 渲染。**Leaflet 真地图引擎 + 彩色阴影地形底图 + 本地 geojson + 多朝代切换**（标杆参考 `community/history-medieval-europe`）。 | (1) `<head>` 引入：`<link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css">` + `<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>` + `<link rel="stylesheet" href="../../scripts/teachany-historical-map.css">`；(2) `</body>` 前 `<script src="../../scripts/teachany-historical-map.js" defer>`；(3) 必须把所需 geojson 从 `skill/assets/historical-{china,world}/<file>.geojson` 复制到课件本地 `assets/maps/`（自包含）；(3.5) **必须把 `skill/assets/hillshade/global-color-hillshade-2k.jpg` 复制为课件本地 `assets/maps/hillshade.jpg`**（205KB 全球 4096×2048 彩色阴影，地图模块默认自动加载 opacity:0.55 叠加，免得出现"暗蓝空地"）；(4) 课件中写 `<div data-teachany-map="my-map" data-teachany-map-scope="china" data-teachany-map-title="秦汉演变"><script type="application/json" data-teachany-map-config>{"eras":[{"id":"qin","label":"秦","file":"qin-dynasty.geojson","fill":"#6366f1","stroke":"#4f46e5","desc":"...","cities":[[34.27,108.95,"咸阳","Xianyang","秦都"]]}], "center":[34,108], "zoom":4, "fitBounds":[[18,72],[52,140]]}</script></div>`。批量工具：`python3 scripts/apply-historical-maps.py` 读 `scripts/historical-maps-manifest.json` 自动注入+复制 geojson+复制 hillshade；(5) 模块自动渲染朝代切换按钮、彩色阴影地形底图、悬停金黄高亮、点击红色城市 popup、时代说明面板、图例。 | ⛔ **严禁**用纯手画 SVG / Canvas 方框拼接当作"历史地图"——城市坐标会和疆域错位；⛔ **严禁**不引入 Leaflet 自造投影；⛔ **严禁**直接 fetch 跨目录 `../../skill/assets/...`——GitHub Pages 部署后 404，必须复制到课件本地；⛔ **严禁**省略 hillshade.jpg——地图会变成"暗蓝空地"；⛔ **严禁**省略时代 `desc`（学生看不懂）；⛔ 新朝代必须先在 `skill/assets/historical-{china,world}/` 补 geojson |
